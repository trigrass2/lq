...
$(function(){
	...
	Ew.search('.searchForm',{
		...
		btnValues:[{
			...
			text:'搜索',
			onClick:function(data){
				$('#tableBasUlocFile').bootstrapTable('refreshOptions',{pageNumber:1});
			}
		},{
			btnId:'btnClear',
			text:'重置',
			tableid:['tableBasUlocFile']
		}]
	});
	...
	Ew.getDictVal(['ROUTE_TYPE', 'ROUTE_STATUS'], function (re) {
		Ew.table('.mainTable',{
			btnValues:[{
				...
				btnId:'btnEdit',text:'编辑',otherOption:[{id:'tableBasUlocFile',selectNum: 1}],onClick:function(){
					var rows = $('#tableBasUlocFile').bootstrapTable('getSelections');
					var flag = true;
				...
				btnId:'btnDelete',text:'删除',isTrue:true,otherOption:[{id:'tableBasUlocFile',selMinNum: 1}],onClick:function(){
					var rows = $('#tableBasUlocFile').bootstrapTable('getSelections');
					ids = [];
					var flag = true;
					$.each(rows,function(index,row){
					...
					if(flag){
						...
						var url = '/base/basUlocFile/delete'
				...
				btnId:'btnDownload',text:'模板下载',isTrue:true,selMinNum:1,onClick:function(){
					window.top.location.href= apiUrl +'/base/basUlocFile/exportTpl'
				}
			},{
				btnId:'btnImport',text:'导入',selMinNum:1,url:'/base/basUlocFile/importFile',tableId:'tableBasUlocFile'
			},{
				btnId:'btnExport',text:'导出',onClick:function(){
					...
					window.top.location.href= apiUrl +'/base/basUlocFile/export?tmBasPlantId='+tmBasPlantId+'&tmBasRouteId='+tmBasRouteId+'&routeType='+routeType+'&status='+status+'&ulocId='+ulocId;
				}
			}],
			tableId:'tableBasUlocFile',
			tableValue:{
				...
				url:'/base/basUlocFile/querylistByPage',
				columns:[{
					checkbox: true
				},{
					field: 'plant',
...
function daliogShow(type){
	...
	var defaultTable=type=='add'?'':'tableBasUlocFile';
	Ew.dialog('mainFromEdit',{
		...
		btnValues:[{
			...
			onClick:function(data){
				if(type=='change'){
					data.tmBasUlocFileId = $('#'+defaultTable).bootstrapTable('getSelections')[0].tmBasUlocFileId;
				}
				datas = JSON.stringify(data);
				var url = (type=='add'?'/base/basUlocFile/add':'/base/basUlocFile/update');
				$.when(Ew.ewAjax(url,datas)).done(function(results){
					$('#mainFromEdit').modal('hide');
					$('#tableBasUlocFile').bootstrapTable('refresh');
				});
			}
		},{
			btnId:'btnCancel',
		...
		form:{
			...
			formList:[{
				...
				idName:'numberDisplayTime',
				text:'显示时间',
				field:'displayTime',
				valid:[{type:"number",min:0,max:3}],
