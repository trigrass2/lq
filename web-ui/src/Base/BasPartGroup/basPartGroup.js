...
$(function(){
	...
	var mainSearchData=[{
		...
		idName:'inputCom52',
		text:'产品组',
		comboUrl:'/plantlayout/basPartgroup/queryPartGroupSuggestAll',
		...
		Ew.table('.demoTable',{
			btnValues:[{
				...
				btnId:'btnDelete',text:'删除',isTrue:true,otherOption:[{id:'table1',selMinNum: 1}],onClick:function(){
					...
					var url = '/plantlayout/basPartgroup/delete';
				...
				btnId:'btnDownload',text:'模板下载',isTrue:true,selMinNum:1,onClick:function(){
					var  url = '/plantlayout/basPartgroup/down';
	           		window.top.location.href = Ew.apiUrl +url;
				}
			},{
				btnId:'btnImport',text:'导入',isTrue:true,selMinNum:1,url:'/plantlayout/basPartgroup/import',tableId:'table1'
			},{
				btnId:'btnExport',text:'导出',isTrue:true,selMinNum:1,onClick:function(){
					var tmBasPlantId = $('#comboplant').val();
					var partgroup = $('#textPartgroup').val();
					var partgroupPart = $('#textPartgroupPart').val();
	
	          		window.top.location.href= apiUrl +'/plantlayout/basPartgroup/export?tmBasPlantId='+tmBasPlantId+'&partgroup='+partgroup+'&partgroupPart='+partgroupPart;
				}
			},{
			...
			tableValue:{
				...
				onLoadSuccess:function(){
					$('.sw').bootstrapSwitch({
						...
						onSwitchChange:function(event,state){
							...
							var url = "/plantlayout/basPartgroup/update";
							$.when(Ew.ewAjax(url,datas)).done(function(results){
				...
				url:'/plantlayout/basPartgroup/querylistByPage',
				columns:[{
					checkbox:true
				},{
					field:'plant',
		...
		Ew.table('.demoTable2',{
			btnValues:[{
				...
				btnId:'btnDeleteSub',text:'删除',isTrue:true,otherOption:[{id:'table2',selMinNum: 1}],onClick:function(){
					...
					var url = '/plantlayout/basPartgroupPart/delete';
					$.when(Ew.ewAjax(url,datas)).done(function(results){
			...
			tableValue:{
				...
				url:'/plantlayout/basPartgroupPart/querylistByPage',
				columns:[{
					checkbox:true
				},{
					field: 'partNo',
	...
	Ew.dialog('demoadd',{
		...
		btnValues:[{
			...
			onClick:function(data){
				...
				var url = (type=='add'?"/plantlayout/basPartgroup/add":"/plantlayout/basPartgroup/update");
				$.when(Ew.ewAjax(url,datas)).done(function(results){
					$('#demoadd').modal('hide');
					$('#table1').bootstrapTable('refresh');
	             });
	...
	Ew.dialog('demoadd',{
		...
		btnValues:[{
			...
			onClick:function(data){
				...
				datas = JSON.stringify(data);
				var url = (type=='add'?"/plantlayout/basPartgroupPart/add":"/plantlayout/basPartgroupPart/update");
				$.when(Ew.ewAjax(url,datas)).done(function(results){
					$('#demoadd').modal('hide');
		...
		form:{
			...
			formList:[{
				...
				field:'partgroupNo',
				valid:[''],
				defaultValue:$('#table1').bootstrapTable('getSelections')[0].partgroupNo,
				disabled:true
			},{
				idName:'textPartgroupNameII',
				text:'产品组名称',
				field:'name',
				valid:[''],
