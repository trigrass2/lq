...
$(function(){
	...
	Ew.search('.searchForm',{
		...
		btnValues:[{
			...
			onClick:function(data){
				console.log(data);
				$('#tableBasUlocScrap').bootstrapTable('refreshOptions',{pageNumber:1});
			}
		},{
			btnId:'btnClear',
			text:'重置',
			tableid:['tableBasUlocScrap']
		}]
	});
	...
	Ew.getDictVal(['ROUTE_TYPE', 'ROUTE_STATUS'], function (re) {
		Ew.table('.mainTable',{
			btnValues:[{
				...
				btnId:'btnEdit',text:'编辑',otherOption:[{id:'tableBasUlocScrap',selectNum: 1}],onClick:function(){
				...
				btnId:'btnDelete',text:'删除',isTrue:true,otherOption:[{id:'tableBasUlocScrap',selMinNum: 1}],onClick:function(){
					var rows = $('#tableBasUlocScrap').bootstrapTable('getSelections');
					ids = [];
					var flag = true;
					$.each(rows,function(index,row){
						if(!(row.routeStatus== 'N' || row.routeStatus== 'M')){
				...
				btnId:'btnDownload',text:'模板下载',isTrue:true,selMinNum:1,onClick:function(){
					window.top.location.href= apiUrl +'/bas/ulocScrap/download'
				}
			},{
				btnId:'btnImport',text:'导入',selMinNum:1,url:'/bas/ulocScrap/importFile',tableId:'tableBasUlocScrap'
			},{
				btnId:'btnExport',text:'导出',onClick:function(){
					...
					var scrapNo = $('#textContentRemark').val();
					window.top.location.href= apiUrl +'/bas/ulocScrap/export?tmBasPlantId='+tmBasPlantId+'&tmBasRouteId='+tmBasRouteId+'&routeType='+routeType+'&status='+status+'&ulocId='+ulocId+'&scrapNo='+scrapNo;
				}
			}],
			tableId:'tableBasUlocScrap',
			tableValue:{
				...
				url:'/bas/ulocScrap/querylistByPage',
				columns:[{
					checkbox: true
				},{
					field: 'plant',
...
function daliogShow(type){
	...
	var defaultTable=type=='add'?'':'tableBasUlocScrap';
	Ew.dialog('mainFromEdit',{
		...
		btnValues:[{
			...
			onClick:function(data){
				if(type=='change'){
					data.tmBasUlocScrapId = $('#'+defaultTable).bootstrapTable('getSelections')[0].tmBasUlocScrapId;
				}
				datas = JSON.stringify(data);
				var url = (type=='add'?'/bas/ulocScrap/add':'/bas/ulocScrap/update');
				$.when(Ew.ewAjax(url,datas)).done(function(results){
					$('#mainFromEdit').modal('hide');
					$('#tableBasUlocScrap').bootstrapTable('refresh');
				});
			}
		},{
			btnId:'btnCancel',
		...
		form:{
			...
			formList:[
			{
				...
				onClick:function(data){
				  $.when(Ew.ewAjax('/system/qsScrap/getScrapInfo',JSON.stringify({tmQmScrapId:data.id}))).done(function(results){
