    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            var key,
                ...
                csses = [],
                data_ = '',
                attributes = {id:item[that.options.treeId]},
                htmlAttributes = [];
      ...
      this.$body.find('> tr[data-index] > td').css('cursor','pointer');
        this.$body.find('> tr[data-index] > td').off('click').on('click', function (e) {
            e.stopPropagation();
            var $this = $(this).find('.tree-icon'),
                $tr = $this.parent().parent(),
                index = $tr.data('index'),
                row = data[index];
