...
(function (factory) {
    ...
    $.widget('mf.manifest', {
        ...
        keys: {
            ...
            RIGHT: 39,
            UP: 38,
            SPACEVAR: 32//Gong娣诲姞 SPACEVAR 绌烘牸 32
        },
        ...
        _create: function () {
            ...
            if (options.marcoPolo) {
                // To prevent Marco Polo from parsing the input's value 鈥� because we
                // want this plugin to parse the value instead 鈥� the value is
        ...
        _bindInput: function () {
            ...
            $input
              .bind('keydown.manifest', function (key) {
                  ...
                  //鐢ㄦ埛鎸変笅閿洏绌烘牸鎴栬€呭洖杞︽椂 楠岃瘉骞舵坊鍔犳暟鎹�
                  if (key.which == self.keys.ENTER || key.which == self.keys.SPACEVAR) {//Gong淇敼 娣诲姞绌烘牸鎸夐挳key.which == self.keys.SPACEVAR
                      key.preventDefault();
                      //Gong娣诲姞浠ｇ爜楠岃瘉閭
        ...
        add: function (data, $mpItem, clearInputValue, initial, Error) {
            var self = this,
                $input = self.$input,
                options = self.options,
            ...
            for (var i = 0, length = values.length; i < length; i++) {
                ...
                //Gong 娣诲姞浠ｇ爜 褰撳墠閭鏍煎紡閿欒鏄坊鍔犵殑li
                if (!email.test(value)) {
                    $item = $('<li class="mf_item mf_itemError" act="error"  title="璇ュ湴鍧€鏍煎紡鏈夐敊璇�" role="option" aria-selected="false" />');
                } else {
                    $item = $('<li class="mf_item " role="option" aria-selected="false" />');
                }
                //Gong娣诲姞End
                // $item = $('<li class="mf_item " role="option" aria-selected="false" />');//Gong娉ㄩ噴
