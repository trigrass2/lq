...
!function ($) {
        ...
        if (this.options.search) {
            ...
            html.push(
                ...
                sprintf('<input class="form-control' + (this.options.iconSize === undefined ? '' : ' input-' + this.options.iconSize) + '" type="text" placeholder="%s" />' +
                  '<span class="glyphicon glyphicon-remove searchRemove">',
    ...
    BootstrapTable.prototype.initSearch = function (){
      var showdatafield = this.columns.map(function(item){
        if(item.field){
          return item.field
        }
        else{
      ...
      showdatafield = showdatafield.filter(function(item){
        return item!= '' && item!= undefined
      });
      var newData = this.data.map(function(item){
        var newitem = {};
        showdatafield.forEach(function(item1){
          for(var i in item){
            if(i == item1){
              newitem[i] = item[i]
            }
                ...
                for (var key in item) {
                    ...
                    if (index !== -1 && that.header.searchables[index] && (typeof value === 'string' || typeof value === 'number')) {
                        if (that.options.strictSearch) {
                          ...
                          var doat = (value + '').toLowerCase();
                          if(doat.toString().indexOf('checkbox') > -1){
                            doat = ''
                          }
                          else if(doat.toString().indexOf('div') > -1){
                            doat = doat.substring(value.toString().indexOf('>') + 1,value.toString().indexOf('</div>'))
                           }
  ...
  BootstrapTable.prototype.updatePagination1 = function (event) {
    ...
    BootstrapTable.prototype.initServer = function (silent, query) {
        ...
        request = $.extend({}, calculateObjectValue(null, this.options.ajaxOptions), {
            ...
            success: function (res) {
              ...
              $('.fixed-table-container thead th .both').removeClass('asc')
              $('.fixed-table-container thead th .both').removeClass('desc')
            },
            error: function (res) {
    ...
    BootstrapTable.prototype.fitHeader = function () {
        ...
        if (that.$el.is(':hidden')) {
            that.timeoutId_ = setTimeout($.proxy(that.fitHeader, that), 0);
            return;
        }
    ...
    var allowedMethods = [
        ...
        'refreshOptions',
        'resetSearch',
        'expandRow', 'collapseRow', 'expandAllRows', 'collapseAllRows','resetHeader'
    ];
