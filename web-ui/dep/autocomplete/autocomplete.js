 ...
 */
 ...
 AutoComplete.prototype = {
			...
			if(keyCode == 38) {
				...
				if(_cache.currentIndex !== -1) {
					...
					if(_config.manySelect) {
						...
						$(_config.hiddenCls,pCls).val(embId).change();
	...
	_doPostAction: function(targetVal,targetParent) {
		...
		// 假如返回的数据如下：
/*		var results = [{name:'name1',code:'code1',id:'1'},{name:'name2',code:'code2',id:'2'},{name:'name3',code:'code3',id:'3'}];
		self._renderHTML(results,targetParent);
        self._executeClick(results,targetParent);*/
		if(id == targetParent.context.id){
			$.ajax({
		        type: 'POST',
		        ...
		        async: false,
		        url:  url,
		        data: targetParent.context.value,
	...
	_renderHTML: function(ret,targetParent) {
		...
		for(var i = 0, ilen = ret.length; i < ilen; i+=1) {
			html += '<p  data-html = "'+ret[i].code+ "-" +ret[i].name+'" embId="'+ret[i].id+'" class="p-index'+i+'">' + 
					   '<span>'+ret[i].code+ "-" +ret[i].name+'</span>' + 
					'</p>';
		}
		// 渲染值到下拉框里面去
	...
	_closed: function(targetParent){
		  ...
		  */
		  $('.alink',targetParent).click(function(){
			 ...
			 $(_config.hiddenCls,curParent).val('').change();
