...
Date.prototype.toInt = function () {
    return this.getFullYear()*10000+(this.getMonth()+1)*100 + this.getHours()*100 +this.getDate();
};
...
Date.prototype.incrementDateByWorkingDays=function (days) {
    ...
    var q = Math.abs(days);
    while (q > 0) {
        this.setHours(this.getHours() + (days > 0 ? 1 : -1));
...
Date.prototype.distanceInWorkingDays= function (toDate){
    ...
    var end=nd.getTime();
    while (pos.getTime() <= end) {
        hours = hours + (isHoliday(pos) ? 0 : 1);
        pos.setHours(pos.getHours() + 1);
    }
    return hours;
};
